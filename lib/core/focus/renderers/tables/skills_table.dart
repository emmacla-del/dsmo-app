...
class SkillsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final renderSpec = SpreadsheetCompiler.compileSkills(
      id: field.id,
      matrix: matrix,
      numRows: numRows,
    );
  ...
  List<List<String>> _buildMatrix(String prefix, int numRows) {
    ...
    for (int i = 0; i < numRows; i++) {
      final rowNum = i + 1;
      matrix.add([
        '${prefix}_skill_${rowNum}_text',
        '${prefix}_skill_${rowNum}_male',
        '${prefix}_skill_${rowNum}_female',
        '${prefix}_skill_${rowNum}_total',
      ]);
    }
