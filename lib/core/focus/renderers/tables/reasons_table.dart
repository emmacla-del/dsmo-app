...
class ReasonsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final renderSpec = SpreadsheetCompiler.compileReasons(
      id: field.id,
      matrix: matrix,
      numRows: numRows,
    );
  ...
  List<List<String>> _buildMatrix(String prefix, int numRows) {
    ...
    for (int i = 0; i < numRows; i++) {
      final rowNum = i + 1;
      matrix.add([
        '${prefix}_reason_${rowNum}_text',
        '${prefix}_reason_${rowNum}_male',
        '${prefix}_reason_${rowNum}_female',
        '${prefix}_reason_${rowNum}_total',
      ]);
    }
