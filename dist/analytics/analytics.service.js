...
let AnalyticsService = class AnalyticsService {
    ...
    async getCompanySummary(companyId, year) {
        const declarations = await this.prisma.declaration.findMany({
            where: {
                year,
                status: prisma_types_1.DeclarationStatus.FINAL_APPROVED,
                companyId,
            },
            include: { employees: true, movements: true },
        });
        ...
        const prevYear = year - 1;
        const prevDeclarations = await this.prisma.declaration.findMany({
            where: { year: prevYear, status: prisma_types_1.DeclarationStatus.FINAL_APPROVED, companyId },
            include: { employees: true },
        });
    ...
    async getCompanyBenchmarks(companyId, year, groupBy = 'sector') {
        ...
        const myDeclarations = await this.prisma.declaration.findMany({
            where: { year, status: prisma_types_1.DeclarationStatus.FINAL_APPROVED, companyId },
            include: { employees: true },
        });
    ...
    calculatePercentile(sortedArray, value) {
